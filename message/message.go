...
// MeshMessage is the 200-byte packed wire-format frame for the Lattice mesh (protocol v5).
// Field order matches the packed C struct — do not reorder without updating the static_assert.
// v3 additions (route_len..auth_tag) append after enrollment_public_key so the
...
type MeshMessage struct {
	...
	// v3: downlink source route (Phase 3) / uplink path accumulator. 48 = MAX_HOPS(8) × 6.
	RouteLen  uint8    `c:"uint8_t"     proto:"12,uint32,optional"`
	RoutePath [48]byte `c:"uint8_t[48]" proto:"13,bytes,optional"`
...
// 127 (v2 prefix) + 1 + 48 + 16 + 8 = 200. Must stay ≤ 200 (ESP-NOW frame limit).
const WireSize = 200
